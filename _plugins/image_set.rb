...
module Jekyll
  class ImageSet < Liquid::Tag
    ...
    def initialize(tag_name, text, tokens)
      ...
      if text =~ /--wrap-class=(.+)/i
        @wrap_class = text.match(/--wrap-class=(.+)/i)[1]
      end
      ...
      if text =~ /--container-id=(\S+)/i
        @container_id = text.match(/--container-id=(\S+)/i)[1]
      end
    end
    ...
    def render(context)
      ...
      # Start building tags
      source = "<#{@container_tag} class='#{@container_class}' id='#{@container_id}'>\n"
